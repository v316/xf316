/*
** $Id: drawtext.c 13762 2011-04-29 07:49:15Z humingming $
**
                ...
                int glyph_type)
{
    ...
    switch (glyph_type) {
		...
		if(ctxt->driton_flag){
			ctxt->advance +=_gdi_draw_new_one_glyph(ctxt->pdc, glyph_value, 
                        (ctxt->pdc->ta_flags & TA_X_MASK) != TA_RIGHT, 
                        ctxt->x, ctxt->y, &adv_x, &adv_y,ctxt->driton_flag);
		}else{		
	...
	if(glyph_value == 0x80000e38 || glyph_value == 0x80000e39 || glyph_value == 0x80000e31 || glyph_value == 0x80000e47 || glyph_value == 0x80000e3a
		||  glyph_value == 0x80000e34 ||  glyph_value == 0x80000e35 ||  glyph_value == 0x80000e36 ||  glyph_value == 0x80000e37
                ...
                RECT* pRect, int indent, UINT nFormat, DTFIRSTLINE *firstline)
{
    ...
    if (nCount < 0){
        nCount = __mg_strlen (pdc->pLogFont, pText);
    }
