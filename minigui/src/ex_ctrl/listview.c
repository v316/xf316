/*
 ** $Id: listview.c 13577 2010-10-26 09:50:09Z houhuihua $
 **
...
#define ID_INERTIAL_FLING_TIMER 2018

// 惯性滑动的物理时间, 值越大, 惯性滑动的时间越久, 距离也越远
#define FLING_PHY_TIME      1000    // msec
// 惯性滑动转换为定时器的时间
#define FLING_TIME(msec)    (msec/10)
...
static struct timeval last_move_time;
static int last_move_start = 0;
static BOOL stop_inertia_move = FALSE;
static float cur_acceleration = 0.0f;
static float duration = 0;
static int direction = 0;
#if USE_AVERAGE // 使用平均值
static int accel_data_count = 0;
#define MAX_ACCEL_DATA_COUNT  128
static float accel_data[MAX_ACCEL_DATA_COUNT];
#endif
        ...
        HDC hdc, PITEMDATA pItem, RECT *rcItem, int nCols)
{
    ...
    if (ph->flags & LVCF_RIGHTALIGN)
        text_format = DT_RIGHT | DT_WORDBREAK;
    else if (ph->flags & LVCF_CENTERALIGN)
        text_format = DT_CENTER | DT_WORDBREAK;
    else
        text_format = DT_LEFT | DT_WORDBREAK;

    RECT calc_rect;
    calc_rect.top = rect_text.top;
    calc_rect.left = rect_text.left;
    calc_rect.bottom = rect_text.bottom;
    calc_rect.right = rect_text.right;

    int text_height = DrawText (hdc, psub->pszInfo, -1, &calc_rect, DT_WORDBREAK|DT_CALCRECT);
    int y_offset = (RECTH(rect_text) - text_height) / 2;
    if (text_height > 0) {
        rect_text.top += y_offset;
        rect_text.bottom += y_offset;
    }
...
#define WPARAM_MASK 0x400 //to get the direction of mouse move
                direction = LOSWORD(wParam) & ~(WPARAM_MASK);

                if (direction == MOUSE_UP) {
                    plvdata->cur_pos += velocity;
                } else if (direction == MOUSE_DOWN) {
                    plvdata->cur_pos -= velocity;
                }

                stop_inertia_move = TRUE;

                struct timeval tv;
                gettimeofday(&tv, NULL);
                uint64_t d_usec = (tv.tv_sec - last_move_time.tv_sec) * 1000000
                    + (tv.tv_usec - last_move_time.tv_usec);
                float speed = (mouseY - last_move_start) * 1000.0f / d_usec; // pixel/msec
                int dir = speed > 0 ? 1 : -1;
                cur_acceleration = - (dir * speed) * 1000.0f / d_usec;
#if USE_AVERAGE // 使用平均值
                ...
                last_move_start = mouseY;
                last_move_time.tv_sec = tv.tv_sec;
                last_move_time.tv_usec = tv.tv_usec;
                ...
                if (plvdata->cur_pos < 0) plvdata->cur_pos = 0;

                return ScrollViewCtrlProc (hWnd, MSG_VSCROLL, SB_THUMBTRACK, plvdata->cur_pos);
            }
        ...
        case MSG_LBUTTONUP:
            {
                ...
                else if (LVSTATUS(hWnd) & LVST_BDDRAG)
                {
                    ...
                    if (move_flag == TRUE) {
                        move_flag = FALSE;
                        if (abs(move_distance) > 5) {
                            stop_inertia_move = FALSE;
#if USE_AVERAGE // 使用平均值
                            float sum = 0.0f;
                            ...
                            duration = cur_acceleration * pow(FLING_TIME(FLING_PHY_TIME), 2) / 2.0f;
                            /* fprintf(stderr, "acceleration: %f, duration: %f\n", cur_acceleration, duration); */
                            if (IsTimerInstalled(hWnd, ID_INERTIAL_FLING_TIMER)) {
                                ResetTimer(hWnd, ID_INERTIAL_FLING_TIMER, 1);
                            } else {
                                SetTimer(hWnd, ID_INERTIAL_FLING_TIMER, 1);
                            }
                            break;
                        }
                    }
        ...
        case MSG_TIMER:
            {
                ...
                if (timer_id == ID_INERTIAL_FLING_TIMER) {
                    static int t = 0;
                    t += 10;

                    float pos = duration - cur_acceleration * powf(t, 2.0f) / 2;

                    if (direction == MOUSE_UP) {
                        plvdata->cur_pos += abs(pos / 10);
                    } else if (direction == MOUSE_DOWN) {
                        plvdata->cur_pos -= abs(pos / 10);
                    }
                    ...
                    if (t == FLING_TIME(FLING_PHY_TIME) || stop_inertia_move) {
                        KillTimer(hWnd, ID_INERTIAL_FLING_TIMER);
                        plvdata->cur_pos = ScrollViewCtrlProc(hWnd, SVM_GETCONTENTY, 0, 0);
        ...
        case LVM_GETSELECTEDITEM:
            {
                ...
                if ( !(pi = LV_GET_ITEM (hsvi)) ) {
                    memset(&p1, 0, sizeof(p1));
                    return 0;
                }

                p1->nItem = scrollview_get_item_index(hWnd, (HSVITEM)pi->addData);
                p1->nItemHeight = scrollview_get_item_height ((HSVITEM)pi->addData);
                p1->itemData = pi->itemData;
                p1->dwFlags = pi->dwFlags;
        ...
        case LVM_CHOOSEITEM:
            {
                ...
                PITEMDATA next_pitem = lvGetItemByRow (plvdata, wParam + 1);
                if (next_pitem) {
                    pitem = next_pitem;
                }
                scrollview_make_item_visible (&plvdata->svdata, 
