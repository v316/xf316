/*
** $Id: iconview.c 13651 2010-11-27 15:30:07Z houhuihua $
**
...
static int direction = 0;//屏幕滑动方向
static int mouse_move_flag = 0;
                     ...
                     MgItem** pRet, int *itemy)
{
    ...
    if(mouseY == mglst->nTotalItemH) {//NOTE:解决图标型控件点击控件最底部位置(mouseY == mglst->nTotalItemH)，无法选中图标条目的问题
        mouseY = mouseY - 1;
    }
...
#define WPARAM_MASK 0x400 //to get the direction of mouse move
        ...
        if(direction == MOUSE_UP) {
            pivdata->ivlist.cur_pos += velocity;
        } else if (direction == MOUSE_DOWN) {
            pivdata->ivlist.cur_pos -= velocity;
        }

        if (pivdata->ivlist.cur_pos < 0) pivdata->ivlist.cur_pos = 0;

        return DefaultScrolledProc (hWnd, MSG_VSCROLL, SB_THUMBTRACK, pivdata->ivlist.cur_pos);//屏幕滑动的处理与滚动条的处理一致
    }

    case MSG_LBUTTONUP:
    case MSG_RBUTTONUP:
    {
        pivdata->ivlist.cur_pos = DefaultScrolledProc(hWnd,SVM_GETCONTENTY,0,0);
        int mouseY = HISWORD (lParam);
        int mouseX = LOSWORD (lParam);
        int nItem;
        RECT rcVis;
        ...
        MgList *mglst = (MgList *)&pivdata->ivlist;
        PSCRDATA pscrdata = pivscr;
        ...
        nItem = mglst->iop.isInItem(mglst, mouseX, mouseY, &hitem, NULL);
        if ( nItem >= 0 ) {
            if ( !(GetWindowStyle(hWnd) & SVS_UPNOTIFY) && (mouse_move_flag == 0)){
                NotifyParentEx (hWnd, GetDlgCtrlID(hWnd), SVN_CLICKED, (DWORD)hitem);
			}
            if (!mglist_is_item_hilight(mglst, hitem) && (mouse_move_flag == 0)) {
    ...
    case MSG_PAINT:
    {
        ...
        mglist_for_each (me, pivlist) {
            ...
            //to paint split line around icon item by clockwise
            ...
            MoveTo(hdc, rcDraw.right, rcDraw.bottom);
