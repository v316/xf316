...
#ifdef _MGIAL_SUNXIKEYTSLIB
	{"sunxikeytslib",InitSUNXIKeyTSLibInput, TermSUNXIKeyTSLibInput},
#endif
...
int mg_InitIAL (void)
{
    ...
    if (GetMgEtcValue ("system", "ial_engine", engine, LEN_ENGINE_NAME) < 0) {
        printf("[file]:%s  [fun]:%s [line]:%d the ial engine is not exit in MiniGUI.cfg, uses the sunxikeytslib by default!!!\n", __FILE__, __func__, __LINE__);
        strcpy(engine, "sunxikeytslib");
        //return ERR_CONFIG_FILE;
    }

	printf("[file]:%s  [fun]:%s [line]:%d the ial engine is %s\n", __FILE__, __func__, __LINE__,engine);
#else /* _MG_MINIMALGDI */
#   ifdef _MGGAL_PCXVFB
...
#   endif
    ...
    if ((env_value = getenv ("MG_MDEV"))) {
        strncpy (mdev0, env_value, MAX_PATH);
        mdev0 [MAX_PATH] = '\0';
    }
    else
#endif

    if (GetMgEtcValue ("system", "mdev0", mdev0, MAX_PATH) < 0) {
        printf("[file]:%s  [fun]:%s [line]:%d the MiniGUI.cfg doesn't exit mdev0!!!!!!!!!\n", __FILE__, __func__, __LINE__);
        mdev0[0] = '\0';
    }
    if (GetMgEtcValue ("system", "mdev1", mdev1, MAX_PATH)) {
        printf("[file]:%s  [fun]:%s [line]:%d the MiniGUI.cfg doesn't exit mdev1!!!!!!!!!\n", __FILE__, __func__, __LINE__);
        mdev1[0] = '\0';
    }
    if (GetMgEtcValue ("system", "mdev2", mdev2, MAX_PATH)) {
        printf("[file]:%s  [fun]:%s [line]:%d the MiniGUI.cfg doesn't exit mdev2!!!!!!!!!\n", __FILE__, __func__, __LINE__);
        mdev2[0] = '\0';
    }
    ...
    strcpy (__mg_cur_input->mdev0, mdev0);
    strcpy (__mg_cur_input->mdev1, mdev1);
    strcpy (__mg_cur_input->mdev2, mdev2);

    if (!IAL_InitInput (__mg_cur_input, mdev0, mtype)) {
