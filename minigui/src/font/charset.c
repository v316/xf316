/*
** $Id: charset.c 13443 2010-09-03 09:35:02Z weiym $
** 
...
//	return (8*94 + 0xFE - 0xA1);
//	return 31*20+19;
//	if((cur_mchar [0] >= 0xA1 && cur_mchar [0] <= 0xFE) && (cur_mchar [1] >= 0xA1 && cur_mchar [1] <= 0xFE))
//		return (cur_mchar [0]-0xa1)*94 + (cur_mchar [1]-0xa1);  //94 = (0xFE-0xA1+1); 
    if (area < 9) {
                ...
                const unsigned char* cur_mchar, int cur_len)
{
 ...
 /*	if(cur_mchar [0] >= 0xA1 && cur_mchar [0] <= 0xFE) 
	{
		if(cur_mchar [1] >= 0x40 && cur_mchar [1] <= 0x7E)
			return (cur_mchar [0]-0xa1)*157+(cur_mchar [1]-0x40);   // 157 = (0x7E-0x40+1)+(0xFE-0xA1+1);
		else if(cur_mchar [1] >= 0xA1 && cur_mchar [1] <= 0xFE)
			return (cur_mchar [0]-0xa1)*157+(cur_mchar [1]-0xa1)+63;  // 63 = (0x7E-0x40+1);
	}
	return 0;*/
}
...
{
#if 1
//	unsigned char cur_mchar1[2]={0x81,0x42};

	if(cur_mchar[0] >= 0x81)
	{
		if(cur_mchar[1] >= 0x41 && cur_mchar[1] <= 0x7E)
			return ((cur_mchar[0]-0x81) * 188 + (cur_mchar[1] - 0x41));   // 188 = (0x7E-0x41+1)+(0xFE-0x81+1);
		else if(cur_mchar[1] >= 0x81 && cur_mchar[1] <= 0xFE)
			return ((cur_mchar[0]-0x81) * 188 + (cur_mchar[1] - 0x81) + 62);  // 62 = (0x7E-0x41+1);
	}
	return 0;
 //   return ((cur_mchar [0] - 0xA1) * 94 + cur_mchar [1] - 0xA1);
#else
...
#if 1  /*modified by huangxaioyang 2014-10-18*/
	if(ch1 >= 0x81 && ch1 <= 0x9F)
	{
		if(ch2 >= 0x40 && ch2 <= 0x7E)
			return (ch1-0x81)*188 + (ch2-0x40);  //188 = (0x7E-0x40+1)+(0xFC-0x80+1); 			
		else if(ch2 >= 0x80 && ch2 <= 0xFC)
			return (ch1-0x81)*188 + (ch2-0x80)+63;  // 63 = 0x7E-0x40+1;			
	}
	else if(ch1 >= 0xE0 && ch1 <= 0xFC)
	{
		if(ch2 >= 0x40 && ch2 <= 0x7E)
			return 5828 + (ch1-0xE0)*188 + (ch2-0x40);  // 5828 = 188 * (0x9F-0x81+1);
		else if(ch2 >= 0x80 && ch2 <= 0xFC)
			return 5828 + (ch1-0xE0)*188 + (ch2-0x80)+63;
	}
	return 0;
                ...
                const unsigned char* cur_mchar, int cur_len)
{
    ...
    int n, t;
	/* yangy@: it will transform uft8 code to unicode(utf16le) */
    if (wc & 0x80) {
        n = 1;
        while (wc & (0x80 >> n))
            n++;
