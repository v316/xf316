...
{
	printf("sunxi_alloc_open");
	
	pthread_mutex_lock(&g_mutex_alloc);
	if (g_alloc_context != NULL)
	{
		printf("sunxi allocator has already been created");
		goto SUCCEED_OUT;
	}
	...
	if (g_alloc_context == NULL)
	{
		printf("create sunxi allocator failed, out of memory");
		goto ERROR_OUT;
	}
	else
	{
...
int sunxi_alloc_close()
{
	...
	if (--g_alloc_context->ref_cnt <= 0)
	{
		...
		list_for_each_safe(pos, q, &g_alloc_context->list)
		{
			...
			printf("sunxi_alloc_close del item phy= 0x%08x vir= 0x%08x, size= %d", tmp->phy, tmp->vir, tmp->size);
			list_del(pos);
			free(tmp);
		}
...
#if (LOG_NDEBUG == 0)
{
		...
		printf("before close allocator, rest size: 0x%08x", rest_size);
}
#endif
...
int sunxi_alloc_alloc(int size)
{
	...
	if (g_alloc_context == NULL)
	{
		printf("sunxi_alloc do not opened, should call sunxi_alloc_open() before sunxi_alloc_alloc(size)");
		goto ALLOC_OUT;
	}
	...
	printf("rest size: 0x%08x", rest_size);

	if (rest_size < size)
	{
		printf("not enough memory, rest: 0x%08x, request: 0x%08x", rest_size, size);
		goto ALLOC_OUT;
	}
	...
	if (0 == addr_phy)
	{
		printf("can not request physical buffer, size: %d", size);
		goto ALLOC_OUT;
	}
	...
	if (alloc_buffer == NULL)
	{
		...
		{
			printf("munmap 0x%08x, size: %d failed", addr_vir, size);
		}
...
void sunxi_alloc_free(void * pbuf)
{
	...
	list_for_each_entry(tmp, &g_alloc_context->list, i_list)
	{
		if (tmp->vir == addr_vir)
		{
			printf("sunxi_alloc_free item phy= 0x%08x vir= 0x%08x, size= %d", tmp->phy, tmp->vir, tmp->size);
...
{
			int rest_size = ioctl(g_alloc_context->fd, SUNXI_MEM_GET_REST_SZ, 0);
			printf("after free, rest size: 0x%08x", rest_size);
}
#endif
	...
	if (0 == flag)
	{
		printf("sunxi_alloc_free failed, do not find virtual address: 0x%08x", addr_vir);
	}
...
int sunxi_alloc_vir2phy(void * pbuf)
{
	...
	list_for_each_entry(tmp, &g_alloc_context->list, i_list)
	{
			...
			&& addr_vir < tmp->vir + tmp->size)
		{
			...
			// printf("sunxi_alloc_vir2phy phy= 0x%08x vir= 0x%08x", addr_phy, addr_vir);
			flag = 1;
			break;
		}
	}
	...
	if (0 == flag)
	{
		printf("sunxi_alloc_vir2phy failed, do not find virtual address: 0x%08x", addr_vir);
	}
...
int sunxi_alloc_phy2vir(void * pbuf)
{
	...
	list_for_each_entry(tmp, &g_alloc_context->list, i_list)
	{
			...
			&& addr_phy < tmp->phy + tmp->size)
		{
			...
			// printf("sunxi_alloc_phy2vir phy= 0x%08x vir= 0x%08x", addr_phy, addr_vir);
			flag = 1;
			break;
		}
	}
	...
	if (0 == flag)
	{
		printf("sunxi_alloc_phy2vir failed, do not find physical address: 0x%08x", addr_phy);
	}
